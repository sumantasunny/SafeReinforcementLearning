...
while counter <= max_count:
    ...
    print("Training Epoch: " + str(counter))
    train_history = dqn.fit(env, nb_steps=steps, visualize=False, verbose=0)
    env.render(mode=False)
    test_history = dqn.test(env, nb_episodes=1, visualize=False, verbose=0)
...
plt.plot(score_log)
plt.show()
np.save("./results/" + str(save_name), score_log)
